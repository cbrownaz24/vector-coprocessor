...
(
  ...
  // consumer dpath sigs (chain mode)
  ...
  // latched cons (cmd2) fields, valid when chain_pending=1
  ...
  reg        chain_pending;  // set on latch, cleared at end of S_EXEC_CHAIN
  ...
  //----------------------------------------------------------------------
  // chain detection (combo)
  //----------------------------------------------------------------------
  // cmd1 and cmd2 must both be plain arith (CAT_VV or CAT_VS, no cmp,
  // no reduce). cmd2 must RAW-dep on cmd1's dst. forbid WAW/WAR.
  ...
  // WAW: cmd2 writes same reg as cmd1
  wire waw_haz = (cmd2_vd == cmd1_vd);

  // WAR: cmd2 writes a reg cmd1 reads. producer reads vs1 every cycle,
  // vs2 only if CAT_VV.
  ...
  // fwd flags computed at detect time
  ...
  always @(*) begin
    ...
    case (state)
      S_IDLE: begin
        if (cmd_val && cmd1_category == CAT_CFG) begin
          if (cmd1_opcode_fn[3:0] == 4'd0) begin // setvl
            ...
            reduce_result = (cmd1_scalar > 32'd32) ? 32'd32 :
      ...
      S_EXEC_CHAIN: begin
        ...
        if (elem_count < vl_reg) begin
          if (mask_reg[elem_count[4:0]]) begin
            // producer is plain arith in chain mode (cmp ruled out)
            vrf_wen = 1'b1;
            cons_vrf_wen = 1'b1;
          end
        end
      ...
      S_REDUCE: begin
        ...
        if (elem_count + 1 >= vl_reg) begin
          // bug fix: include last elem contrib. acc update is registered
          // so on the final cycle reduce_acc only has VL-1 elems folded
          // in. add in-flight vs1_rdata combo for the published result.
          reduce_val = 1'b1;
  ...
  // helper: decode arith cmd alu_fn
  // (cons cant be a cmp in chain mode so simpler)
        ...
        S_IDLE: begin
          if (cmd_val) begin
            ...
            if (chain_possible) begin
              ...
              chain_pending <= 1'b0;
              // plain non-chained dispatch, orig logic
              case (cmd1_category)
                CAT_VV: begin
                  if (cmd1_is_reduce)
                ...
                CAT_CFG: begin
                  if (cmd1_opcode_fn[3:0] == 4'd0)
                    vl_reg <= (cmd1_scalar > 32'd32) ? 6'd32 :
                              cmd1_scalar[5:0];
                  state <= S_IDLE;
                end
              endcase
