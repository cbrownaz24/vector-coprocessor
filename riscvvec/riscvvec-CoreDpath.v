...
(
  ...
  // Vector bypass mux outputs (for vector command packing in Core)
  output [31:0] op0_byp_mux_out_Dhl_out,
  output [31:0] op1_byp_mux_out_Dhl_out,
  ...
  // Expose bypass mux outputs for vector command packing
  assign op0_byp_mux_out_Dhl_out = op0_byp_mux_out_Dhl;
  assign op1_byp_mux_out_Dhl_out = op1_byp_mux_out_Dhl;
  ...
  // Register File
  // Mux write port: scalar writeback has priority, but vector reduction
  // can write when scalar is not writing.

  wire        rf_wen_actual   = rf_wen_Whl || vec_reduce_val;
  wire [ 4:0] rf_waddr_actual = rf_wen_Whl ? rf_waddr_Whl : vec_reduce_rd;
  wire [31:0] rf_wdata_actual = rf_wen_Whl ? wb_mux_out_Whl : vec_reduce_result;
  ...
  (
    ...
    .wen_p   (rf_wen_actual),
    .waddr_p (rf_waddr_actual),
    .wdata_p (rf_wdata_actual)
  );
